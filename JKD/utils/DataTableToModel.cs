    ...
    private static List<T> GetList<T>(this DataTable data, string prefix, bool ignoreCase = true) where T : new()
    {
        ...
        try
        {
            ...
            var rowscount = data.Rows.Count;
                Dictionary<string, string> dict = new T().GetType().GetField("TitleMapFiled").GetValue(BindingFlags.Static) as Dictionary<string, string>;
                for (int i = 0; i < rowscount; i++)
            {
                ...
                foreach (var p in properties)
                {
                     ...
                     string newKN =    dict.Where(item => item.Key == p.Name).Select(it => it.Value).FirstOrDefault();
                    var keyName = prefix + (string.IsNullOrEmpty(newKN)?p.Name:newKN) + "";
                        if (ignoreCase)
